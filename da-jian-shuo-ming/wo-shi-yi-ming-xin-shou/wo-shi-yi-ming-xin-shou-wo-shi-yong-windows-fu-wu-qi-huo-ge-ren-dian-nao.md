...
######################监控系统#####################
#此处填写端口后，允许外网访问此端口。然后将布置机器和此端口交到群162279609中，可以生成服务状态监控页面
PrometheusPort=1260
######################邮箱信息#####################
...
NotFoundSkill=请使用".en技能"的方式进行成长，注意后面不要加数字\n本骰子不再支持无意义的.en 数字。必须在录入人物卡后才可以正常en技能
#dnd
...
#云黑
notBanListInput=只有相应黑名单条目的录入者才可以删除
...
其中必填信息，尤其是要修改hostIp、javaPort、serverPort三项要和上方的HttpApi插件对齐。而下方的master、heap、cloudBan则自选修改。
...
cloudBan是云黑开关，设为false则不使用云黑。

heap是心跳报告开关，当你使用服务器配置骰娘时，将此项开启，一旦骰娘挂起，会在10分钟内，在骰主群中收到报警通知（骰主群号为808619122）

master与cloudBan\heap相关，填写骰主QQ
